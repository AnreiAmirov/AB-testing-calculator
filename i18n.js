...
const T = {
  en: {
    ...
    c2_pitfall:'First read: 6,200 in control vs 5,800 in variant. The Analyse tab\'s Sample Ratio Mismatch check flags it — p ≈ 0.0004, far below the 0.001 alarm. A redirect bug was dropping some users before assignment. <strong>Any result here would be untrustworthy.</strong> They fix the bug and rerun.',
    ...
    c2_analyse:'Clean rerun: control 30.0% vs variant 32.5%, a healthy 50/50 split, z ≈ 4.2, p < 0.001.',
    ...
    c3_plan:'Revenue per visitor is a continuous value, not yes/no, so this needs <strong>Welch\'s t-test</strong>, not a Z-test — and the spread (standard deviation), not just the average, drives the sample size. With high variance the calculator asks for ~23,700 per group. Traffic is only ~1,500/day, so the lesson flips: rather than a holdout, they must run <strong>~5 weeks</strong> just to reach the minimum.',
    ...
    c3_run:'After 5 weeks: ~43,000 visitors per arm. Control mean $18.00, variant mean $19.15 (both with large standard deviations ~$42).',
    c3_analyse:'Welch\'s t-test: t ≈ 4.0, p < 0.001. The +$1.15 per visitor is small relative to the spread, but with 43,000 per arm it\'s clearly distinguishable from noise.',
  ...
  ru: {
    ...
    c2_pitfall:'Первое чтение: 6 200 в контроле против 5 800 в варианте. Проверка SRM во вкладке «Анализ» это ловит — p ≈ 0,0004, намного ниже порога 0,001. Баг с редиректом терял часть пользователей до распределения. <strong>Любой результат здесь недостоверен.</strong> Баг чинят и перезапускают.',
    ...
    c2_analyse:'Чистый перезапуск: контроль 30,0% против варианта 32,5%, здоровый сплит 50/50, z ≈ 4,2, p < 0,001.',
    ...
    c3_plan:'Выручка на посетителя — непрерывная величина, не «да/нет», поэтому нужен <strong>t-тест Уэлча</strong>, а не Z-тест — и размер выборки определяет разброс (стандартное отклонение), а не только среднее. При высокой дисперсии калькулятор просит ~23 700 на группу. Трафик всего ~1 500/день, поэтому урок переворачивается: вместо холдаута придётся крутить <strong>~5 недель</strong>, чтобы лишь набрать минимум.',
    ...
    c3_run:'Через 5 недель: ~43 000 посетителей на группу. Контроль в среднем $18,00, вариант $19,15 (у обоих большое стандартное отклонение ~$42).',
    c3_analyse:'t-тест Уэлча: t ≈ 4,0, p < 0,001. +$1,15 на посетителя мал относительно разброса, но при 43 000 на группу чётко отличим от шума.',
