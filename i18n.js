...
const T = {
  en: {
    ...
    srm_ok:'Split looks healthy: arm sizes are consistent with an even split (p = {p}).',
    srm_bad:'Sample Ratio Mismatch: arm sizes differ from an even split (p < 0.001). Assignment may be broken — results unreliable until fixed.',
  ...
  ru: {
    ...
    srm_ok:'Сплит выглядит здоровым: размеры групп согласуются с равным делением (p = {p}).',
    srm_bad:'Несоответствие долей (SRM): размеры групп отличаются от равного деления (p < 0,001). Назначение может быть сломано — результатам нельзя доверять, пока не исправлено.',
    ...
    c1_problem:'SaaS-команда рассылает еженедельный дайджест 200 000 подписчиков. Open rate — 22%. Маркетинг считает, что тема с интригой («Вы упускаете 3 функции») побьёт текущую. Нужны доказательства до раскатки на всю базу.',
    ...
    c1_analyse:'Z-тест для двух долей: z ≈ 3,1, p ≈ 0,002. 95% интервалы двух open rate не пересекаются, сплит был чистый 50/50 (без проблем с SRM).',
    ...
    c4_title:'Подсказка в онбординге меняет поведение', c4_metric:'Ratio',
    ...
    c4_graph_a:'контроль', c4_graph_b:'вариант', c4_graph_cap:'Каждая точка — ratio пользователя. Облако варианта выше, но дельта-метод учитывает именно разброс по пользователям.',
    ...
    c4_take:'Вывод: ratio-метрики (действий/сессию, выручка/заказ) требуют дельта-метода для верной дисперсии — иначе наловите ложных срабатываний.',
