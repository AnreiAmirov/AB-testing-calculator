...
const T = {
  en: {
    ...
    nav_home:'Home',
    home_brand:'Data for Everyone',
    home_tagline:'Statistics tools that explain themselves — free, bilingual, and running entirely in your browser.',
    home_pick:'What do you want to do?',
    home_pick_sub:'Two toolkits for making confident decisions from data. Pick where to start.',
    home_ab_cta:'Open the calculator →',
    home_foot:'Free · No signup · Nothing leaves your device · English & Русский',
  ...
  ru: {
    ...
    home_causal_title:'Каузальный вывод',
    ...
    home_causal_cta:'Перейти к каузальному выводу →',
    home_foot:'Сделано продуктовым аналитиком как открытый двуязычный обучающий ресурс. Всё работает на стороне клиента.',
    // ── Страница каузального вывода ──
    causal_title:'Каузальный вывод',
    causal_sub:'A/B-тест — золотой стандарт. Но в половине реальных задач его нельзя провести: нельзя рандомизировать цену, откатить уже выкаченную фичу или разделить город надвое. Здесь начинается каузальный вывод.',
    c_toc_h:'Содержание',
    ...
    c_why_lead:'Каузальный вывод — это набор методов, которые оценивают причинный эффект по данным, полученным <em>без</em> рандомизации.',
    ...
    c_why_key_b:'Суть:', c_why_key:'A/B-тест отвечает на «сработает ли, если запустить». Каузальный вывод — на «сработало ли то, что уже произошло» и «что будет, если развернуть на всех». Это не замена тестам, а инструмент для случаев, где тест недоступен.',
    ...
    c_idea_p2:'Поэтому любой метод каузального вывода — это, по сути, разный способ <strong>построить правдоподобный контрфакт</strong>: найти группу или период, который честно показывает «что было бы без изменения». Хороший анализ = хороший контрфакт. Плохой контрфакт = красивая, но ложная цифра.',
    ...
    c_choose_key_b:'Правило по умолчанию:', c_choose_key:'если можно провести настоящий эксперимент — проводите. Каузальный вывод дороже в обосновании и всегда опирается на допущения, которые нельзя проверить до конца.',
    c_limits_h:'8 · Честные ограничения — чтобы не обмануть себя',
    c_limits_p:'Каузальный вывод мощен, и именно поэтому опасен: он выдаёт уверенно выглядящую цифру, даже когда допущения не выполнены. Что держать в голове:',
    ...
    c_limits_key_b:'Главная мысль раздела:', c_limits_key:'ценность аналитика не в том, чтобы посчитать эффект, а в том, чтобы честно сказать, <em>насколько</em> ему можно верить. Хороший каузальный анализ всегда открыто называет своё слабое место.',
